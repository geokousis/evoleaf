...
export default async function (webRouter, privateApiRouter, publicApiRouter) {
  ...
  webRouter.use(function (req, res, next) {
    ...
    if (!Array.isArray(res.locals.nav.header_extras)) {
      res.locals.nav.header_extras = []
    }
    const isLoggedIn = SessionManager.getSessionUser(req.session) != null
    if (isLoggedIn) {
      const hasInstallNavItem = res.locals.nav.header_extras.some(
        item => item?.url === '/install'
      )
      if (!hasInstallNavItem) {
        res.locals.nav.header_extras.push({
          text: 'install',
          url: '/install',
          trackingKey: 'install',
          class: req.path.startsWith('/install') ? 'active' : '',
