...
async function _buildRequest(projectId, options) {
  ...
  if (options.incrementalCompilesEnabled || options.syncType != null) {
    ...
    timer.done()
    const rangesMap = await _getProjectRangesMap(projectId)
    return _finaliseRequest(projectId, options, project, docs, files, rangesMap)
  }
}
...
async function _buildRequestFromDocupdater(
  ...
  const rangesMap = await _getProjectRangesMap(projectId)
  return _finaliseRequest(projectId, options, project, docs, [], rangesMap)
}
...
/**
 * Fetch tracked-change ranges for all docs in a project.
 * Returns a map of docId (string) -> ranges object.
 * Silently returns an empty map on failure so compilation still works.
 */
async function _getProjectRangesMap(projectId) {
  try {
    const rangeDocs =
      await DocumentUpdaterHandler.promises.getProjectRanges(projectId)
    if (!rangeDocs) return {}
    const map = {}
    for (const rangeDoc of rangeDocs) {
      map[rangeDoc.id] = rangeDoc.ranges
    }
    return map
  } catch (err) {
    logger.warn(
      { projectId, err },
      'Failed to fetch project ranges for compile; compiling with full content'
    )
    return {}
  }
}
...
/**
 * Build compile content from the "true edit" document state by undoing all
 * pending tracked suggestions:
 * - tracked insertions are removed
 * - tracked deletions are reinserted
 *
 * Process in descending position order so offsets remain valid. For identical
 * positions, process insertion-removals first, then deletion-reinsertions.
 */
function _stripTrackedSuggestions(content, changes) {
  const tracked = (changes || [])
    .filter(c => c.op && typeof c.op.p === 'number')
    .sort((a, b) => {
      if (b.op.p !== a.op.p) return b.op.p - a.op.p
      const aIsInsert = typeof a.op.i === 'string' && a.op.i.length > 0
      const bIsInsert = typeof b.op.i === 'string' && b.op.i.length > 0
      if (aIsInsert === bIsInsert) return 0
      return aIsInsert ? -1 : 1
    })

  for (const change of tracked) {
    const pos = change.op.p
    if (typeof change.op.i === 'string' && change.op.i.length > 0) {
      const len = change.op.i.length
      content = content.slice(0, pos) + content.slice(pos + len)
      continue
    }
    if (typeof change.op.d === 'string' && change.op.d.length > 0) {
      content = content.slice(0, pos) + change.op.d + content.slice(pos)
    }
  }
  return content
}

function _finaliseRequest(
  projectId,
  options,
  project,
  docs,
  ...
  const compileWithSuggestions =
    options?.suggestionCompile === true || options?.suggestionCompile === 'true'
  const resources = []
  let flags = []
  ...
  for (let path in docs) {
    ...
    if (doc.lines != null) {
      ...
      // Default compile strips pending suggestions; optional mode keeps them.
      const docId = doc._id?.toString()
      const changes = rangesMap[docId]?.changes
      let content = doc.lines.join('\n')
      if (!compileWithSuggestions && changes && changes.length > 0) {
        const beforeLength = content.length
        content = _stripTrackedSuggestions(content, changes)
        logger.info(
          {
            projectId,
            docId,
          ...
          'compile: stripped tracked suggestions from resource content'
        )
      }
      resources.push({
        path,
